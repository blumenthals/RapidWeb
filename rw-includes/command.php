...
function rw_POST_upload_image_ajax($request, $response) {
    ...
    if($file->size == 0) {
        ...
        imagecopyresampled($imgsquare, $img, 0, 0, ($w - $s) / 2, ($h - $s) / 2, 150, 150, ($w - $s) / 2 + $s, ($h - $s) / 2 + $s);
        imagejpeg($imgsquare, $dir.'/'.$request['img']->name.".150x150.jpg");

        $response->renderJSON(array('$insertAll' => array('gallery' => array(array('image' => $vdir.'/'.$request['img']->name, 'thumbnail' => $vdir.'/'.$request['img']->name.".150x150.jpg")))));
    }
}
